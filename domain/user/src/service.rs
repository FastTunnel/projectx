...
use crate::facade::{IConfigFacade, IJwtFacade};
use crate::publisher::{IUserEventPublisher, UserEvent};
use crate::repository::{
    IOrganizationRepository, IRoleRepository, ITeamRepository, IUserRepository,
};

use super::model::{
    CreateOrganizationParam, CreateRoleParam, CreateTeamParam, CreateUserParam, Organization, Role,
    Team, User,
};
    ...
    T: Send + Sync,
{
    ...
    async fn init_system(
        ...
        // init roles
        let mut roles = Role::init_system_roles(&organization.identifier);
        self.role_repo.save_all(tx, &mut roles).await?;
