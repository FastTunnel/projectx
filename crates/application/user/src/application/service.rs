...
use app_interface::user::dto::command::{
...
use app_interface::user::dto::{OrganizationDto, RoleDTO, UserDto};
use app_interface::user::IUserAppService;
use app_interface::{error, DbTx, APP_STATE};
use domain_user::model::{
    CreateOrganizationParam, CreateRoleParam, CreateTeamParam, CreateUserParam, Organization, User,
};
use domain_user::repository::IRoleRepository;
use domain_user::service::IUserService;

pub struct UserAppService {
    user_service: Arc<dyn IUserService<DbTx>>,
    role_repository: Arc<dyn IRoleRepository<Transaction = DbTx>>,
}
...
impl IUserAppService for UserAppService {
        ...
        self.user_service
            .create_role(
                ...
                CreateRoleParam {
                    ...
                    is_project_role: command.is_project_role,
                    own: command.own.clone(),
                },
            )
            .await?;
        transaction.commit().await?;
    ...
    async fn query_roles_by_own(
        ...
        let roles = self
            .role_repository
            .query_roles_by_org_and_own(&mut transaction, organization, own)
            .await?;
        transaction.commit().await?;
        Ok(roles.into_iter().map(|v| v.into()).collect())
    }
}
